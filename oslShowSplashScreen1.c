...
OSL_VIRTUALFILENAME __osl_logo_ram_files[] =		{
...
int oslShowSplashScreen1()			{
	...
	oslAddVirtualFileList(__osl_logo_ram_files, oslNumberof(__osl_logo_ram_files));

	texte = oslLoadImageFile("ram:/logo/texte.png", OSL_IN_VRAM, OSL_PF_8888);
	if (!texte)
		return 0;
	...
	etoile = oslLoadImageFile("ram:/logo/etoile.png", OSL_IN_RAM | OSL_SWIZZLED, OSL_PF_5551);
	if (!etoile)		{
		oslDeleteImage(texte);
		oslDeleteImage(fond);
		return 0;
...
//			oslSysBenchmarkDisplay();
	...
	oslRemoveVirtualFileList(__osl_logo_ram_files, oslNumberof(__osl_logo_ram_files));
	return 1;
}
